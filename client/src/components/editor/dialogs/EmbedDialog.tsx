...
interface EmbedDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onInsert: (url: string, caption?: string, provider?: string) => void;
  initialUrl?: string;
...
export function EmbedDialog({
  ...
  const detectProvider = (inputUrl: string): string => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError('Please provide a valid embed URL');
      return;
    }
    ...
    onInsert(trimmed, caption.trim() || undefined, provider);
    onClose();
  };
      ...
      <div className="relative w-full max-w-[540px] flex flex-col bg-[#F8F7F3] border border-[#DDD9D0] rounded-2xl shadow-2xl p-6 sm:p-7 animate-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between pb-4 border-b border-[#DDD9D0]">
          <div className="flex items-center gap-2">
            <Code2 className="w-4 h-4 text-[#716D65]" />
            <h2 className="font-editorial text-2xl text-[#211E1A]">Interactive Embed</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="https://codepen.io/... or Spotify, Figma, Loom"
              autoFocus
            ...
            <p className="text-xs text-[#716D65] mt-1.5">
              Supports CodePen, Spotify, Figma, Loom, GitHub Gist, and secure web embeds.
            </p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Interactive CodePen prototype"
