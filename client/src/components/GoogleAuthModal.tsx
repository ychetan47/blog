...
export function GoogleAuthModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsPending(true);
    try {
      await api.auth.devLogin({ email, name });
      await refreshUser();
      onClose();
      if (isSignUp) {
        navigate('/onboarding');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl border border-stone-200 overflow-hidden z-10 animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8">
          ...
          {/* Primary Action: Direct Google OAuth 2.0 Web Server Flow */}
          <button
            type="button"
            onClick={loginWithGoogle}
            className="w-full flex items-center justify-center gap-2.5 py-3 px-4 rounded-full bg-[#1A73E8] hover:bg-[#1557B0] text-white text-xs font-medium transition-colors shadow-xs cursor-pointer mb-6"
          >
            <span>Launch Google Authorization</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-3 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-6 py-2.5 rounded-full bg-[#211E1A] hover:bg-stone-800 disabled:opacity-50 text-white text-xs font-medium transition-colors shadow-xs cursor-pointer"
              >
